                            ...
                            <div class="media align-items-center gap-10">
                                ...
                                <div class="media-body">
                                    ...
                                    @if(isset($cartItem['variations']) && !empty($cartItem['variations']))
                                        ...
                                        @php($branchProduct = \App\Model\ProductByBranch::where(['product_id' => $cartItem['id'], 'branch_id' => session('branch_id')])->first())
                                        ...
                                        @foreach($cartItem['variations'] as $cartVariation)
                                            ...
                                            @endif
                                        @endforeach
                                    @elseif(!empty($cartItem['variant']))
                                        @php($totalVariationPrice = isset($cartItem['variation_price']) ? $cartItem['variation_price'] : 0)
                                        <small class="text-muted">
                                            {{Str::limit($cartItem['variant'], 50)}}
                                            @if($totalVariationPrice > 0)
                                                <span class="text-success">(+{{Helpers::set_symbol($totalVariationPrice)}})</span>
                                            @else
                                                <span class="text-muted">({{Helpers::set_symbol(0)}})</span>
                                            @endif
                                        </small>
                                    @endif
